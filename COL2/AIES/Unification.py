...
def Unify(L1, L2, testset):
    ...
    for i in range(len(L1.args)):
        ...
        if S == False:
            return False
        if S != None:
            SUBSET[S[0].values] = S[1].value
    
    return SUBSET

# if __name__ == "__main__":
    
print(Unify(Rel("Knows", [Constant("Raj"), Variable("X")]),
            Rel("Knows", [Variable("Y")]),
            Rel("Sister", [Variable("Y")])))
        
print()
print(Unify(Rel("Knows", [Constant("Raj"), Variable("X")]),
            Rel("Knows", [Variable("Y") ,Constant("Sheetal")])))

print()
print(Unify(Rel("Knows", [Constant("Raj"), Variable("X")]),
            Rel("Knows", [Variable("X"), Constant("Sheeta")])))